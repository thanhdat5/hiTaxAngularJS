(function (app) {
	...
	departmentListController.$inject = ['$scope', '$rootScope', 'apiService', '$ngBootbox', 'notificationService', 'authData'];
	function departmentListController($scope, $rootScope, apiService, $ngBootbox, notificationService, authData) {
		// Set page title
		$rootScope.pageTitle = "Department Management";
		...
		$scope.mainGridOptions = {
			toolbar: ["excel", "pdf"],
			excel: {
				fileName: "hiTax_Departments.xlsx",
			...
			pdf: {
				...
				repeatHeaders: true,
				scale: 0.8,
				fileName: "hiTax_Departments.pdf",
			},
			dataSource: {
				type: "json",
				transport: {
		...
		$scope.saveItem = function () {
			...
			if (validator.validate()) {
				if ($scope.popupModel.Id) {
					apiService.put('/api/departments/Update', $scope.popupModel,
						function (success) {
					...
					apiService.post('/api/departments/Add', $scope.popupModel,
						function (success) {
		...
		$scope.deleteItem = function (dataItem) {
			$ngBootbox.confirm('Are you sure to remove the selected record?')
				.then(function () {
					...
					apiService.del('/api/departments/delete', config,
						function (response) {
...
})(angular.module('hiTax.departments', ["kendo.directives"]));
