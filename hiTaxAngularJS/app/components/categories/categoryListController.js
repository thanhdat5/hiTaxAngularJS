(function (app) {
	...
	categoryListController.$inject = ['$scope', '$rootScope', 'apiService', '$ngBootbox', 'notificationService', 'authData'];
	function categoryListController($scope, $rootScope, apiService, $ngBootbox, notificationService, authData) {
		// Set page title
		$rootScope.pageTitle = "Category Management";

		// Set data gridview
		$scope.mainGridOptions = {
			toolbar: ["excel", "pdf"],
			excel: {
				fileName: "hiTax_Categories.xlsx",
		...
		$scope.saveItem = function () {
			...
			if (validator.validate()) {
				if ($scope.popupModel.Id) {
					apiService.put('/api/categories/Update', $scope.popupModel,
						function (success) {
					...
					apiService.post('/api/categories/Add', $scope.popupModel,
						function (success) {
		...
		$scope.deleteItem = function (dataItem) {
			$ngBootbox.confirm('Are you sure to remove the selected record?')
				.then(function () {
					...
					apiService.del('/api/categories/delete', config,
						function (response) {
...
})(angular.module('hiTax.categories', ["kendo.directives"]));
